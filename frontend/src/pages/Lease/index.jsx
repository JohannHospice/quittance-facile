...
import { getLeases } from "../../service/airtable/receipt";
...
export default function LeasePage() {
  const { data: records, error, loading } = useFetch([], getLeases);
  ...
  return (
    <Layout>
      <Box mb={3}>
        <Typography variant="h4">Les bails</Typography>
      </Box>
      {loading ? (
        <CircularProgress />
      ) : error ? (
                ...
                description={
                  <>
                    <Typography variant="body1">
                      {record.data.receipts.length} quittances
                    </Typography>
                  </>
                }
                onClick={() => {
      ...
      <Box position="absolute" bottom="72px" right={0} margin={3}>
        <Fab
          color="primary"
          aria-label="add"
          onClick={() => {
